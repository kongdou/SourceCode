...
class ReduceTask extends Task {
  ...
  /**
   * ReduceTask的Shuffle和Merger阶段的实现
   * @author deepsky
   *
   * @param <K>
   * @param <V>
  ...
  class ReduceCopier<K, V> implements MRConstants {
    ...
    class ShuffleRamManager implements RamManager {
      ...
      public ShuffleRamManager(Configuration conf) throws IOException {
        //Reduce Task中大部分内存用于缓存从Map Task端拷贝的数据分片
    	//设置存储这些值的内存占比
   ...
   //数据拷贝线程
    ...
    public ReduceCopier(TaskUmbilicalProtocol umbilical, JobConf conf,
      ...
      //拷贝数据容错机制，如果拷贝数据出错次数超过abortFailureLimit，杀死Redduce Task
      this.abortFailureLimit = Math.max(30, numMaps / 10);
      //拷贝数据容错机制,如果拷贝数据次数超过maxFetchFailuresBeforeReporting
      //进行检查，决定是否杀死Reduce Task
      ...
      //ShuffleRamManager中已使用内存超过此设置，且内存文件数目超过2个
      //合并内存中文件，默认为0.66
        ...
        while (copiedMapOutputs.size() < numMaps && mergeThrowable == null) {
          ...
          synchronized (scheduledCopies) {
            ...
            // all reduce-tasks swamping the same tasktracker
        	  //防止出现数据访问热点（大量线程集中读取某个TaskTracker上的数据）
        	  //ReduceTask不会直接将列表mapLocations中的MapTask输出数据位置交给
        	  //MapOutputCopier线程，而是事先进行一次预处理：将所有TaskTracker Host进行混洗操作
        	  //然后保存到scheduledCopies列表中，而MapOutputCopier线程将从该列表中获取待copy的Map Task
        	  //输出数据位置
      ...
      public void run() {
        try {
          ...
          while(!exitLocalFSMerge){
            synchronized (mapOutputFilesOnDisk) {
            //文件数目超过2 * ioSortFactor - 1） --ioSortFactor为合并文件文件配置
            //时，线程LocalFSMerge会从mapOutputFilesOnDisk中取最小的ioSortFactor个
            //文件进行合并，合并后重新再次写到磁盘上	
              while (!exitLocalFSMerge &&
    ...
    //该线程周期性的通过RPC从TaskTracker上获取已经完成的Map Task列表
    //并将成功运行完成的Map Task放到列表mapLocations中
