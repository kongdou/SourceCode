 ...
 * TaskTraceker向JobTracker发送节点运行时的信息，包括TaskTracker基本信息
 * 节点资源使用情况和各个任务状态等
 * 每次给JobTracker发送心跳时，TaskTracker会重新构造该类
 * 每次发送的信息中部分信息将不相同
 **************************************************/
public class TaskTrackerStatus implements Writable {
  ...
  //TaskTracker主机名
  String host;
  //TaskTracker对外的HTTP端口
  int httpPort;
  //TaskTracker上已经失效的总数
  int failures;
  //当前TaskTracker上各个任务运行状态
  List<TaskStatus> taskReports;
  //上次汇报心跳的时间
  volatile long lastSeen;
  //Map solt（插槽）总数，即允许同时运行的Map Task总数，由参数设定
  private int maxMapTasks;
  //Reduce solt（插槽）总数，即允许同时运行的Reduce Task总数，由参数设定
   ...
   * TaskTracker资源（CPU、内存等信息）
   */
  static class ResourceStatus implements Writable {

    private long totalVirtualMemory;    //总可用的虚拟内存量，单位为byte
    private long totalPhysicalMemory;	//总的可用物理内存量
    private long mapSlotMemorySizeOnTT;    //每个Map Solt对应的内存量
    private long reduceSlotMemorySizeOnTT;	//每个Reduce Solt对应的内存量
    private long availableSpace; //可用磁盘空间
    
    private long availableVirtualMemory = UNAVAILABLE; // in byte 可用虚拟内存量
    private long availablePhysicalMemory = UNAVAILABLE; // in byte 可用物理内存量
    private int numProcessors = UNAVAILABLE;	//节点总的处理器数
    private long cumulativeCpuTime = UNAVAILABLE; // 运行以来累计的CPU使用时间（毫秒）
    private long cpuFrequency = UNAVAILABLE; // CPU主频（KHZ）
    private float cpuUsage = UNAVAILABLE; // CPU使用率（%）
