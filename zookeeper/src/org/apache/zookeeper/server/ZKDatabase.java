...
public class ZKDatabase {
    ...
    public ZKDatabase(FileTxnSnapLog snapLog) {
    	//ZK内存数据的核心模型，像一棵树，保存了ZK上的所有节点信息
    ...
    public long loadDataBase() throws IOException {
    	//初始化工作第三步：创建PlayBackListener监听器，主要用来接收事务应用过程中的回调
       //PlayBackListener是一个事务应用监听器，用于在事务应用过程中的回调：每当成功
    	//将一条事务日志应用到内存数据库中，就会调用这个监听器
