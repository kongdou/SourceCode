...
public class FileTxnSnapLog {
	...
	private static final Logger LOG = LoggerFactory
			.getLogger(FileTxnSnapLog.class);
	...
	public long restore(DataTree dt, Map<Long, Integer> sessions,
		...
		//初始化工作第五-七步：获取最新的100个快照、逐个解析快照、获取最新的ZXID
		...
		while (true) {
			...
			//初始化工作第九步：获取所有zxid_for_snap之后提交的事务
			hdr = itr.getHeader();
			if (hdr == null) {
				// empty logs
			...
			if (hdr.getZxid() < highestZxid && highestZxid != 0) {
				LOG.error(highestZxid + "(higestZxid) > " + hdr.getZxid()
						+ "(next log) for type " + hdr.getType());
			} else {
				//获取比步骤7中得到大的事务
				highestZxid = hdr.getZxid();
			}
			try {
				//初始化工作第10步：获得大于zxid_for_snap的事务后，将其逐个应用到之前
				//基于快照数据文件恢复出来的DataTree和sessionWithTimeouts中去
