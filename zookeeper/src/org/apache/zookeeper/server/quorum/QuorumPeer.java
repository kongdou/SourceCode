...
public class QuorumPeer extends Thread implements QuorumStats.Provider {
    ...
    synchronized public void startLeaderElection() {
    	try {
    		//初始化Leader选举（自身服务器ID，最新的ZXID，当前服务器的epoch）
    		//大概算法为集群中哪台机器处理的数据最新（通常根据每个服务器处理过的最大
    		//ZXID来比较确定是否数据为最新），越有可能成为leader，如果zxid一样
    		//sid最大的就是leader
        ...
        //根据选举类型创建选举算法,目前只支持FastLeaderElection算法
